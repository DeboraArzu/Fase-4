using System;
using System.Collections.Generic;//libreria para estructuras en C# <list, arraylist, diccionary>
using System.Linq;
using System.Text;
using System.Threading.Tasks;
//librerias para el manejo de archivos, expresiones regulares
...
namespace Fase_3_Compiladores__tablas_
{
    class Analisis
    {
        ...
        Dictionary<string, Dtoken> tokens;
        ...
        public string analizar()
        {
            ...
            try
            {
                ...
                {
                    this.obfilacolumna();//manera momentanea de mostrar errores
                ...
                else
                {
                    ...
                    {
                        pos++;
                        mensaje = validarID(mensaje, 0);//0 -> id(\s*).
                        ...
                        else
                        {
                            ...
                            {//si en dado caso biene
                                ...
                                {
                                    pos++;
                                    mensaje = this.validarID(mensaje, 1);//id , id | id.
                                    if (mensaje.CompareTo("") != 0)
                                    {
                                        return mensaje;
                                    }
                ...
                {
                    int prediccion = this.adivinarpalabra();//ve que biene sin avanazar en la pos actual
                    if (prediccion == 0)
                    {
                        #region caso 0
                        obfilacolumna();
                    ...
                    {//id + = (manejo de los tokens) //cuando estoso comienzan bien
                        ...
                        vinoTokens = true;
                        Dtoken tk = new Dtoken(nameTokinio, cadena, sucheck);
                    ...
                    {//comienzan los operadores <'> o "
                        ...
                        {
                            op = new tokenoperadores("left");
                            pp = "left";
                        }
                        else if (this.poseeRight(temporal))
                        {
                            op = new tokenoperadores("right");
                            pp = "right";
                        }
                        else
                        {
                        ...
                        this.operadores.push(elements, op.sside);
                ...
                if (mypeek == 8 && poseecheck == true)//seccion de keywords
                {
                    ...
                    pos++;
                    mensaje = this.validarID(mensaje, 2);
                ...
                int VendraComs = this.adivinarpalabra();
                if (VendraComs == 9)
                {
                    welcomeComments = true;
                }
        ...
        string validarID(string s, int val)
        {
            string resp = "";
            try
            {
        ...
        string Validar_keywords(bool b)
        {
            try
            {
        ...
        public Dictionary<string, string> TOK()
        {
            ...
            foreach (string to in tokens.Keys)
            {
                string contenido = tokens[to].contenido;
                string check = tokens[to].check;
                prueba.Add(to, contenido + "‡" + check);
            }
            return prueba;
        }

        public Dictionary<string, string> CONJ()
        {
            //0Ĭ9
            Dictionary<string, string> prueba = new Dictionary<string, string>();
            ...
            {
                string conjunto = "";
                foreach (string conte in conjuntos[con])
                {
                    if (conte.Contains("Ĭ"))
                    {
                        string[] datos = conte.Split('Ĭ');
                        conjunto += "'" + datos[0] + "'..'" + datos[1] + "'" + "+";
                    }
                    else
                    {
                        conjunto += "'" + conte + "'" + "+";
                ...
                prueba.Add(con, "{" + conjunto + "}");
            }
            return prueba;
        }
        public Dictionary<string, int> getContenidos()
        {
            Dictionary<string, int> da = new Dictionary<string, int>();
            foreach (string key in Simbolos.Keys)
            {
                int numero = Simbolos[key].numtoken;
                da.Add(key, numero);
            }
            return da;
        }
